...
public:
    ...
    __aicore__ inline void Init(GM_ADDR tilingGM, GM_ADDR x, GM_ADDR y)
    {
        ...
        xGM.SetGlobalBuffer((__gm__ T*)x + vecTiling._blockOffset, vecTiling._blockLength);
        yGM.SetGlobalBuffer((__gm__ T*)y + vecTiling._blockOffset, vecTiling._blockLength);
...
private:
    ...
    __aicore__ inline void Compute(uint32_t len)
    {
        ...
        switch (tilingData.threshType)
        {
            ...
            case 3:
                Compare(mask, xLocal, yLocal, CMPMODE::GT, len);
                Select(yLocal, mask, xLocal, static_cast<T>(0), SELMODE::VSEL_TENSOR_SCALAR_MODE,
                       len);
                break;
            case 4:
                Compare(mask, xLocal, yLocal, CMPMODE::LE, len);
